...
class ResFinancialYear(models.Model):
    _name = 'res.financial.year'
    ...
    def _get_current_financial_year(self):
        ...
        records = self.search([])
        for rec in records:
            if rec.date_from.year == curr_year:
                return rec


class ResFinancialYearLinkEnd(models.Model):
    _name = 'res.financialyearlinkend'     #dbo.lkpfinancialyearlinkend.csv
    _description = "Financial Year Organisation Link End"
    _order = "id desc"
    ...
    def _get_current_financial_year(self):
        ...
        records = self.search([])
        for rec in records:
            return rec if rec.date_from.year == curr_year else False
