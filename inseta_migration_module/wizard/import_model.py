...
class ImportRecords(models.TransientModel):
	...
	file_type = fields.Selection([('csv', 'CSV File'),('xls', 'XLS File')], default='csv', string='File Type')
	data_file = fields.Binary(string="Upload File (.xls)")
	filename = fields.Char("Filename")
	model_name = fields.Char("Model")

	import_type = fields.Selection([
			('inseta_generic', 'Inseta generic data'),
			('employee', 'Employee'),
			('customer', 'Customer'),
			('vendor', 'Vendor'),
			('warehouse', 'Warehouse'),
	...
	def inseta_migration_button(self):
		if self.import_type == "inseta_generic":
			file_data = self.excel_reader(0)
			# this includes all data that has no relational tables such as equity and disability
			model= self.env[self.model_name]
				
			for row in file_data:
				data = {'legacy_system_id': int(row[0]), 'name': row[1],  "active": True} 
				if self.model_name in ['res.nationality', 'res.means.identification']:
					data["saqacode"] = row[2]  
				else:
					data["code"] = row[2]
				rec = model.search([('name', '=', data.get('name'))])
				if rec:
					rec.write(data)
				else:
					model.create(data)
	...
	def create_contact_person_record(self, fname, mname, lname, contact_person_phone=False, contact_person_email=False):
		first_name, middle_name, second_name = fname.capitalize() if fname else "", mname if mname else "", lname if lname else ""
		fullname = ' '.join([first_name, middle_name, second_name])
		partner_obj = self.env['res.partner']
		partner_rec = partner_obj.search([('name', '=ilike', fullname),
	...
	def create_partner_record(self, customer_id, company_type, customer_rank, supplier_rank, name, phone =False, second_phone=False, \
		whatsapp_no= False, email=False, second_email=False, website=False, tin_no = False, street1=False, street2=False, city=False, lga=False, \
			zone=False, title=False, cfname=False, cmname=False, clname=False, cphone=False, cemail=False, sale_person_arg=False):
		partner_obj = self.env['res.partner']
		...
		if name:
			...
			if partner_id:
				...
				lga_ref = lga_obj.search([('name', '=', lga)], limit=1)
				zone_id = zone_obj.search([('name', '=', zone)], limit=1)
				if lga or zone:
					if not lga_ref:
					...
					if not zone_id:
						...
						pass 
				state_id = lga_ref.state_id
				region_id = zone_id.region_id
				country_id = region_id.country_id
				title_obj = self.env['res.partner.title']
				title = title_obj.search([('name', '=ilike', title)], limit=1)
				# if sale_person_arg:
				sale_person_id = False 
				if sale_person_arg:
					emp_ref = self.env['hr.employee'].search([('emp_code', '=', sale_person_arg)], limit=1)
					sale_person_id = emp_ref.user_id.id
				partner_id = partner_obj.create({
					'name': name, 'phone': phone, 'email': email, 
					'whatsapp_no': whatsapp_no, 'mobile': second_phone,
					'company_type': company_type, 'lga_id': lga_ref.id, 'zone_id': zone_id.id,
					'state_id': state_id.id, 'country_id': country_id.id, 'secondary_email' :second_email,
					'website': website, 'tin_no': tin_no, 'user_id': sale_person_id,
					'region_id': region_id.id, 'street': street1, 'street2': street2, 'city': city,
					'title': title.id if title else False, 'customer_id': customer_id,
					'supplier_rank':supplier_rank, 'customer_rank': customer_rank, 'lastname': "-",  'lastname2': "-", 
					'firstname': "-",
					'child_ids': [(4, self.create_contact_person_record(cfname, cmname, clname, cphone, cemail))]
				})
				return partner_id.id
	...
	def import_contacts(self, index):
		if self.data_file:
			file_datas = base64.decodestring(self.data_file)
			workbook = xlrd.open_workbook(file_contents=file_datas)
		...
		for row in file_data:
			# import sheet 1
			self.generate_record_sheet(row, True)
	...
	def generate_record_sheet(self, row, sheet):
		...
		phone = str(int(primary_phn_num)) if primary_phn_num else None
		partner_id = None 
			...
			partner_id = self.create_partner_record(customer_id, company_type, customer_rank, False, fullname, phone, secondary_phn_num, \
				whatsapp_phn_num, email, \
					second_email, website, False, address1, address2, city, lga, zone, False, cfname, cmname, \
						clname, cphone, cemail, sales_id_col)
			if opening_balance:
				self.generate_move(partner_id, opening_balance,False)
		else:
			partner_id = self.create_partner_record(customer_id, company_type, False, supplier_rank, fullname, phone, secondary_phn_num, \
				whatsapp_phn_num, email, second_email, website, row[12], address1, address2, city, False, False, False, cfname, cmname, \
					clname, cphone, cemail, False)
	...
	def import_records_action(self):
		...
		if self.import_type == "employee":
			...
			self.import_contacts(1)

		elif self.import_type == "customer":
			self.import_contacts(0)
...
class MigrationDialogModel(models.TransientModel):
	...
	_description ="Import Confirmation Dialog"
